...
proof -
  ...
  then have "\<forall> p. p \<in> prime_factors (k^2) \<longrightarrow> even (multiplicity p a)"  using EvenMultRelPrimes GCDab1 by blast
...
lemma odddiv: "(n::nat) \<noteq> 0 \<Longrightarrow> d dvd (2*n) \<Longrightarrow> odd d \<Longrightarrow> d dvd n"
  by (metis coprime_nat dvd_productE dvd_triv_left gcd_2_odd mult.left_neutral)
...
proof-
  ...
  from auv aodd have uodd: "odd u"  by (metis even_power even_times_iff)
  from auv aodd have vodd: "odd v"  by (metis even_power even_times_iff)
  ...
  from gcduv1 auv obtain r s::nat where rdef: "r^2 = u" and sdef: "s^2 = v"  using a0 sqProdnat by blast
