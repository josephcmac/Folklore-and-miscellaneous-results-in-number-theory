...
lemma TrapezoidalNumbersNec2_5recA:
  "\<forall> d. (odd t \<and> d*(t::nat) = 2^r \<longrightarrow> t = 1)"
proof (induction r)
  case 0
  then show ?case 
    by simp
next
  case (Suc r)
  then show ?case 
    by (smt Suc_mult_cancel1 dvdE even_multD even_numeral even_power numeral_2_eq_2 power_Suc semiring_normalization_rules(18) zero_less_Suc)
qed
...
next
  ...
  then have \<open>t \<ge> 2\<close> 
   ...
   from  \<open>d*t = 2 ^ Suc r\<close> have "even t \<or> even d" 
     by (metis even_multD even_numeral even_power zero_less_Suc)
   then show ?thesis
   proof (cases "even t")
     case True
     then show ?thesis 
