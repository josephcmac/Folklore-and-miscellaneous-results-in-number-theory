    ...
    have "\<exists> x::nat. a\<^sub>2 = a\<^sub>1 + 2*x" 
     ...
     have  rdef: "\<exists> r::nat. r*Y = a\<^sub>1 + d*X"
       proof -
         ...
         then have "Y dvd (a\<^sub>1 + x)" using gcdXY by (metis coprime_dvd_mult gcd.commute) 
...
corollary  FermatChristmasUniquenessUpPerm :
  fixes p a\<^sub>1 b\<^sub>1 a\<^sub>2 b\<^sub>2 :: nat
  assumes pprime: "prime p"
    and sumofsq1: "p = a\<^sub>1^2 + b\<^sub>1^2"
    and sumofsq2: "p = a\<^sub>2^2 + b\<^sub>2^2"
  shows "(a\<^sub>1 = a\<^sub>2 \<and> b\<^sub>1 = b\<^sub>2) \<or> (a\<^sub>1 = b\<^sub>2 \<and> b\<^sub>1 = a\<^sub>2)"
  by (smt FermatChristmasUniqueness Suc_1 add.commute even_add even_power le_SucE le_add2 less_add_eq_less linorder_not_less nat_less_le one_add_one pos2 power_eq_one_eq_nat pprime prime_power_iff prime_prime_factor sumofsq1 sumofsq2 two_is_prime_nat)
